...
pocc = function(a0, a1, sand) {								
	...
	# Exponential weights as a function of sand
	w = exp(a0 + sand*a1)									
	
	# Apply dirichlet with weights=w
	p = apply(sand, MARGIN = 2, FUN = rdirichlet, n = 1)
	colnames(p) = colnames(sand)
	rownames(p) = rownames(sand)	
	return(p)
}	
