...
write.table(sand, "Data/sand_smm.txt")
write.table(statseq, "Data/statseq_smm.txt")
write.table(substat, "Data/subseq_smm.txt")
...
for(i in 1:nfish) {
	...
	ml = paste("Fish", i, " - click anywhere to advance to the next fish")
	plot(statseq[i,], type = 'l', lwd = 2, ylim = c(1,nstates), xlab = xl, ylab = yl, main = ml)
	lines(seq(ndays), ceiling(sim.jags$q50$out[i,]/pcs), lwd = 2, lty = 2)
	polygon(x = c(seq(ndays),rev(seq(ndays))), y = ceiling(c(sim.jags$q2.5$out[i,]/pcs,rev(sim.jags$q97.5$out[i,]/pcs))), border = NA, col = rgb(0,0,0,0.5))
...
for(i in 1:nfish) {
	...
	ml = paste("Fish", i, " - click anywhere to advance to the next fish")
	plot(statseq[i,], type = 'l', lwd = 2, ylim = c(1,nstates), xlab = xl, ylab = yl, main = ml)
	lines(seq(ndays), ceiling(ndhmm.jags$q50$out[i,]/pcs), lwd = 2, lty = 2)
	polygon(x = c(seq(ndays),rev(seq(ndays))), y = ceiling(c(ndhmm.jags$q2.5$out[i,]/pcs,rev(ndhmm.jags$q97.5$out[i,]/pcs))), border = NA, col = rgb(0,0,0,0.5))
	points(seq(ndays), actobs[i,], pch = 9, lwd = 2)
	legend("topright", bty = 'n', c("Actual", "Median Estimated", "Observations", "95% CI"), lty = c(1,2,NA,1), pch = c(NA,NA,9,NA), lwd = c(2,2,2,10), col = c(1,1,1,rgb(0,0,0,0.5)))
	locator(1)
}
...
simdf = data.frame(fish = factor(rep(seq(nfish),ndays)), 
	day = rep(seq(ndays),each=nfish), 
	state = c(allobs),
	sub = c(actsub))
write.table(simdf, "Data/simdf.txt")

# Run model and choose number of knots
modlist = kchoose(df = simdf, obs = allobs, sub = actsub, sand = sand, tol = 0.01)
...
# Check the output
summary(sim.gam$model)
plot(sim.gam$model, pages = 1)
gam.check(sim.gam$model)
...
# Add covariates
mcldf$day = day = rep(rep(seq(ndays),each=nfish), each = nstates*pcs)
...
write.table(mcldf, "Data/smmmcldf.txt")

# Run occupancy model
sim.mcl = mclogit(count|state.day.ind ~ sand, weights = weight, random = ~0+sand|ind, data = mcldf)

# Check output
summary(sim.mcl)

# Save models
sim.gm = list(sim.gam = sim.gam, sim.mcl = sim.mcl)
save(object = sim.gm, file = "Output/simgam.rda")
...
# Store fish counts per state in KxN table with K states and N days
...
chain1 = ST.CARar(c(carobs) ~ c(sandmat), family = "binomial", W = W, trials = c(nfobs), 
	burnin = 50000, n.sample = 1050000, thin = 100, AR = 1)
...
sim.car = list(chain1, chain2, chain3)
save(object = sim.car, file = "Output/simcar.rda")
